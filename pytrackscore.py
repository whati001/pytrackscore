...
class pytrackscore():
	def __init__(self, *args, **kwargs): 
		...
		#self.writeWorksheetInitial('Generated_2GROUPS_2TEAMS','8:30','7','5','10','8','10','JA',gamemode='MIXEDGROUP')
		#self.getTeamNames(40)
		#self.getTeamNames(1)
		#self.getTeamNames(2)
		#self.getTeamNames(3)
		#self.getTeamNames(4)
		#self.writeMatchValue(1,0,4)
		#self.writeMatchValue(2,4,0)
        	#self.writeMatchValue(2,3,6,'8:35:00')
		#self.writeMatchValue(57,1,3)
		#self.writeMatchValue(1,10,5,getfieldname="YES")
		#self.writeMatchValue(2,20,45)
		#self.writeMatchValue(63,1,62)
		#self.writeMatchValue(64,23,62)
	...
	def writeWorksheetInitial(self, *args, **kwargs):
		...
		###Value in Fields
		for self.game_count_finally in range(1,int(len(self.groups)*(len(self.groups[0])-1))+1):
			if self.game_count_finally %2 == 0:
			...
			for idx in range(5):
				#Format Positiongames field
				self.trackingsheet[get_column_letter(column_index_from_string(self.startfield_finally[0])+idx)+str(int(self.startfield_finally[1])+(self.game_count_finally)-1)].fill = PatternFill(patternType='solid', start_color=color)
				self.trackingsheet[get_column_letter(column_index_from_string(self.startfield_finally[0])+idx)+str(int(self.startfield_finally[1])+(self.game_count_finally)-1)].border = Border(left=Side(border_style='thin', color='FF000000'), right=Side(border_style='thin', color='FF000000'), top=Side(border_style='thin', color='FF000000'), bottom=Side(border_style='thin', color='FF000000'))
								
			###Value in Fields
			self.trackingsheet[get_column_letter(column_index_from_string(self.startfield_finally[0]))+str(int(self.startfield_finally[1])+(self.game_count_finally)-1)] = self.game_count_finally
	...
	def writeMatchValue(self, *args, getfieldname="NO"):
		...
		if match_count <= self.game_count:
			...
			##Delete logic, check via OrderArray, all games until value "None" was found, then we have delete all newer games##
			for game_pos in range(match_count+1, len(self.order)):
				group_del = int(self.order[game_pos-1].split('-')[0])
				teampositionA_del = int(self.order[game_pos-1].split('-')[1])
				teampositionB_del = int(self.order[game_pos-1].split('-')[2])
				if self.trackingsheet[str(get_column_letter(self.startfield_block[0]+teampositionB_del)+str(int(self.startfield_block[1])+int(group_del)*(len(self.groups[int(group_del)])+1)+teampositionA_del))].value == None:
					break;
				else:
					#Delete from Groupview
					self.trackingsheet[str(get_column_letter(self.startfield_block[0]+teampositionB_del)+str(int(self.startfield_block[1])+int(group_del)*(len(self.groups[int(group_del)])+1)+teampositionA_del))] = None
					self.trackingsheet[str(get_column_letter(self.startfield_block[0]+teampositionA_del)+str(int(self.startfield_block[1])+int(group_del)*(len(self.groups[int(group_del)])+1)+teampositionB_del))] = None
					#Delete from Matchview
					self.trackingsheet[self.gamescore_fields[game_pos-1][1]] = None
					self.trackingsheet[self.gamescore_fields[game_pos-1][2]] = None
					
		#CC or PG Games, now I do not need to fill the Grouptable at all, so only Groupview
		else:	
			print(match_count)
			print(self.gamescore_fields)
			field = re.split('(\d+)',self.gamescore_fields[match_count-1][1])
			print(field)
			self.trackingsheet[field[0]+str(int(field[1]))] = value_teamA
			self.trackingsheet[get_column_letter(column_index_from_string(field[0])+1)+str(int(field[1]))] = value_teamB
		#Save Workbook
		self.workbook.save(filename = self.workbook_path)

		#return fieldname if getfieldname == Yes
		if getfieldname == "YES":
			homefield = ["".join(x) for _, x in itertools.groupby(self.gamescore_fields[match_count-1][1], key=str.isdigit)]
			guestfield = ["".join(x) for _, x in itertools.groupby(self.gamescore_fields[match_count-1][2], key=str.isdigit)]
			#if we play groupgames, we have to update/return the fieldnumber from the groupview
			if match_count <= self.game_count:
				groupview01 = str(get_column_letter(self.startfield_block[0]+teampositionB)+str(int(self.startfield_block[1])+int(group)*(len(self.groups[int(group)])+1)+teampositionA))
				groupview02 = str(get_column_letter(self.startfield_block[0]+teampositionA)+str(int(self.startfield_block[1])+int(group)*(len(self.groups[int(group)])+1)+teampositionB))
				#print("Groupview")
				#print(get_column_letter(self.startfield_block[0]+teampositionB-self.startfield_block[0]+1)+str(int(self.startfield_block[1])+int(group)*(len(self.groups[int(group)])+1)+teampositionA-int(self.startfield_block[1])))
				#print(get_column_letter(self.startfield_block[0]+teampositionA-self.startfield_block[0]+1)+str(int(self.startfield_block[1])+int(group)*(len(self.groups[int(group)])+1)+teampositionB-int(self.startfield_block[1])))
				print([homefield[0]+str(int(homefield[1])-int(self.startfield_group[1])+2),guestfield[0]+str(int(guestfield[1])-int(self.startfield_group[1])+2),str(get_column_letter(self.startfield_block[0]+teampositionB-self.startfield_block[0]+1)+str(int(self.startfield_block[1])+int(group)*(len(self.groups[int(group)])+1)+teampositionA-int(self.startfield_block[1]))),str(get_column_letter(self.startfield_block[0]+teampositionA-self.startfield_block[0]+1)+str(int(self.startfield_block[1])+int(group)*(len(self.groups[int(group)])+1)+teampositionB-int(self.startfield_block[1])))])
				return [homefield[0]+str(int(homefield[1])-int(self.startfield_group[1])+2),guestfield[0]+str(int(guestfield[1])-int(self.startfield_group[1])+2),str(get_column_letter(self.startfield_block[0]+teampositionB-self.startfield_block[0]+1)+str(int(self.startfield_block[1])+int(group)*(len(self.groups[int(group)])+1)+teampositionA-int(self.startfield_block[1]))),str(get_column_letter(self.startfield_block[0]+teampositionA-self.startfield_block[0]+1)+str(int(self.startfield_block[1])+int(group)*(len(self.groups[int(group)])+1)+teampositionB-int(self.startfield_block[1])))]
			else:
				print([homefield[0]+str(int(homefield[1])-int(self.startfield_group[1])+2),guestfield[0]+str(int(guestfield[1])-int(self.startfield_group[1])+2)])
				return [homefield[0]+str(int(homefield[1])-int(self.startfield_group[1])+2),guestfield[0]+str(int(guestfield[1])-int(self.startfield_group[1])+2)]
	...
	def finishGame(self):
		...
		print("Startfield for Postionlist is: "+str(self.startfield_finally))
		places = int((len(self.groups[0])-1)*len(self.groups))
		print("Teamcount is: "+str(places))
		place_count = 0
		for count01 in range(0,int(places/2)):
			...
			#Get the total goal scores from the teams	
			for count02 in range(0,len(self.gamescore_fields)):
				#print(self.gamescore_fields[count02][0])
				#if right team from positiongame list (for example...4|09:02|JVP Hofstätten|3|0|Fireball Hafning -> Fireball Hafning) wins
				...
				#if left team from positiongame list (for example...4|09:02|JVP Hofstätten|3|0|Fireball Hafning -> JVP Hofstätten) wins
			...
			#Check with team has won and write values to excel sheet.
			if self.trackingsheet[self.gamescore_fields[int(self.game_count+self.game_count_cc+int(places/2)-count01-1)][1]].value > self.trackingsheet[self.gamescore_fields[int(self.game_count+self.game_count_cc+int(places/2)-count01-1)][2]].value:
				...
				#Goals First Team			
				self.trackingsheet[get_column_letter(column_index_from_string(self.startfield_finally[0])+2)+str(int(self.startfield_finally[1])+place_count)] = goals[2]
				self.trackingsheet[get_column_letter(column_index_from_string(self.startfield_finally[0])+3)+str(int(self.startfield_finally[1])+place_count)] = goals[3]
				self.trackingsheet[get_column_letter(column_index_from_string(self.startfield_finally[0])+4)+str(int(self.startfield_finally[1])+place_count)] = goals[2] - goals[3]
			
				#Goals Second Team
				self.trackingsheet[get_column_letter(column_index_from_string(self.startfield_finally[0])+2)+str(int(self.startfield_finally[1])+place_count+1)] = goals[0]
				self.trackingsheet[get_column_letter(column_index_from_string(self.startfield_finally[0])+3)+str(int(self.startfield_finally[1])+place_count+1)] = goals[1]
				self.trackingsheet[get_column_letter(column_index_from_string(self.startfield_finally[0])+4)+str(int(self.startfield_finally[1])+place_count+1)] = goals[0] - goals[1]
	...
	def getScore(self, playedmode, mode='array'):
		...
		if playedmode == "Group":
			startvalue = int(self.startfield_group[1])-2
			endvalue = int(re.split('(\d+)',self.gamescore_fields[int(self.game_count-1)][0])[1])
			columncount = 6
		elif playedmode == "CC-four":
			startvalue = int(re.split('(\d+)',self.gamescore_fields[int(self.game_count)][0])[1])-3
			endvalue = int(re.split('(\d+)',self.gamescore_fields[int(self.game_count-1+self.game_count_cc-4)][0])[1])
			columncount = 6
		elif playedmode == "CC":
			startvalue = int(re.split('(\d+)',self.gamescore_fields[int(self.game_count)][0])[1])-3
			endvalue = int(re.split('(\d+)',self.gamescore_fields[int(self.game_count-1+self.game_count_cc)][0])[1])
			columncount = 6
		elif playedmode == "PG":
			startvalue = int(re.split('(\d+)',self.gamescore_fields[int(self.game_count+self.game_count_cc)][0])[1])-3
			endvalue = int(re.split('(\d+)',self.gamescore_fields[int(self.game_count-1+self.game_count_cc+self.game_count_pg)][0])[1])
			columncount = 6
		elif playedmode == "TP":
			print(self.game_count_finally)
			startvalue = int(re.split('(\d+)',self.gamescore_fields[int(self.game_count-1+self.game_count_cc+self.game_count_pg)][0])[1])+2
			endvalue = int(re.split('(\d+)',self.gamescore_fields[int(self.game_count-1+self.game_count_cc+self.game_count_pg)][0])[1])+self.game_count_finally+4
			columncount = 5
		...
		if mode == 'array':
			...
			#Save Games
			#For every row
			for idx in range(int(startvalue), int(endvalue)+1):
				#Add new array
				self.getscore[1].append([])
				#fill new array with all column values
				for idx2 in range(columncount):
					#if first column (Gamecount) convert to int, I do not know why, but by CC and PG it copy otherwise a dezimal value
					print(idx2)
					print(idx)
					print(self.trackingsheet[get_column_letter(self.startfield_group[0]+idx2)+str(idx)].value)
					if (idx2 == 0) and (idx >= (int(startvalue)+3)) and (self.trackingsheet[get_column_letter(self.startfield_group[0]+idx2)+str(idx)].value != None):
						self.getscore[1][idx-startvalue].append(int(self.trackingsheet[get_column_letter(self.startfield_group[0]+idx2)+str(idx)].value))
					else:
						self.getscore[1][idx-startvalue].append(self.trackingsheet[get_column_letter(self.startfield_group[0]+idx2)+str(idx)].value)
			...
			#for every row
			for idx in range(int(startvalue), int(endvalue)+1):
				#for every column values
				for idx2 in range(columncount):
					#if first column (Gamecount) convert to int, I do not know why, but by CC and PG it copy otherwise a dezimal value
					if (idx2 == 0) and (idx >= (int(startvalue)+3)) and (self.trackingsheet[get_column_letter(self.startfield_group[0]+idx2)+str(idx)].value != None):
						self.getscore = self.getscore +str(int(self.trackingsheet[get_column_letter(self.startfield_group[0]+idx2)+str(idx)].value))+','
					#if not the first and not the last one
					elif idx2 != 5:
						self.getscore = self.getscore +str(self.trackingsheet[get_column_letter(self.startfield_group[0]+idx2)+str(idx)].value)+','
					#if it is the last one, do not add the ',' at the end, otherwise the split is not working correctly by the webpage.
					else:
						self.getscore = self.getscore +str(self.trackingsheet[get_column_letter(self.startfield_group[0]+idx2)+str(idx)].value)
				if idx != endvalue:
